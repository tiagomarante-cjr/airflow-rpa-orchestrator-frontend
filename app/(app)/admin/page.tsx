...
export default async function AdminPage() {
  ...
  const users = Object.entries(permissions).map(([email, dags]) => ({
    email,
    dags,
  }));
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Manage which DAGs each user can access and what actions they can perform.{" "}
          <span className="font-medium text-slate-700">
            {users.length} user{users.length !== 1 ? "s" : ""}
          </span>
          ,{" "}
          <span className="font-medium text-slate-700">
            {allDagIds.length} DAG{allDagIds.length !== 1 ? "s" : ""}
          </span>{" "}
          available.
        </p>
      </div>
